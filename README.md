...
## Building and Running Alpaca.cpp in Docker

Install Rancher Desktop or another docker-compatible tool.

```sh
make docker
```

If using `containerd` and `nerdctl` do this to build the image:

```sh
make DOCKER_CLI=nerdctl docker
```

Then you mount the binary files directory into your container and run like this:

```sh
docker run --rm -it -v$HOME/model_data/:/models -w /models alpaca_cpp
```

This mounts your `$HOME/model_data/` directory into your container
as `/models` and changes the working directory to `/models` and runs
the image's entrypoint (`/usr/local/bin/chat`).

Alternatively, you can pull and run this [up-to-date image](https://hub.docker.com/r/kayvan/alpaca.cpp) from Docker Hub.
